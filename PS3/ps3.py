...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        x, y = map(math.floor, [pos.get_x(), pos.get_y()])
        self.dirts[x][y] = max(0, self.dirts[x][y] - capacity)
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        pos = self.position.get_new_position(self.direction, self.speed)
        if not self.gets_faulty() and self.room.is_position_valid(pos):
            self.position = pos 
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:
            self.direction = random.randint(0, 359)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for _ in range(num_trials):
        ...
        for i in count(1):
            for r in robots: r.update_position_and_clean()
            coverage = room.get_num_cleaned_tiles() / room.get_num_tiles()
            if coverage >= min_coverage: sims.append(i); break
